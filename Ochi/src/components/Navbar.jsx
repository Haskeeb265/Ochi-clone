...
function Navbar() {
  ...
  return (
    <div className={`fixed z-[999] w-full px-20 py-8 flex justify-between items-center bg-white bg-opacity-10 backdrop-blur-md shadow-md transition-transform duration-300 ${visible ? 'transform translate-y-0' : 'transform -translate-y-full'}`}>
      <div className='logo'>
      ...
      <div className='links flex gap-10'>
        {["Services", "Our work", "About us", "Insights", "Contact Us"].map((item, index) => (
          <a key={index} className={`font-["Neue_Montreal"] text-lg capitalize text-black ${index === 4 && "ml-40"}`}>
            {item}
          </a>
        ))}
      </div>
